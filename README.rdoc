...
* Codes: http://github.com/alkalinecoffee/boris
* Email: sharkwavemedia@gmail.com
...
Let's pull some information from a RedHat Enterprise Linux server on our network:
  ...
  # let's use a helper to suggest how we should connect to it (which is
  # useful if we're not sure what kind of device this is)
  ...
  if target.connected?
    ...
    # detect which profile to load up (is this target running windows? solaris? or what?)
    target.detect_profile

    puts target.target_profile

    target.retrieve_all
    puts target.to_json(true)
  end
