...
class HPOnboardAdministratorTest < BaseTestSetup
  context 'an HP Onboard Administrator target' do
    setup do
      @connector = @target.connector = SSHConnector.new(@host, {})
      @target.stubs(:target_profiler).returns(Profilers::HPOnboardAdministratorCore)
      @target.force_profiler_to(Profilers::HPOnboardAdministratorCore)
      @profiler = @target.profiler
      @connector.stubs(:values_at).with('show enclosure info').returns(['onboard administrator'])
    end

    should 'detect when a target should use the HPOnboardAdministrator profile' do
      assert_equal(Profilers::HPOnboardAdministratorCore, @profiler.class)
    end
