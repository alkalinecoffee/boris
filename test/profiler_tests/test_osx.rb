...
class OSXCoreTest < ProfilerTestSetup
  context 'an OS X target' do
    setup do
      @connector = @target.connector = SSHConnector.new(@host, {})
      @target.stubs(:target_profiler).returns(Profilers::OSX)
      @target.force_profiler_to(Profilers::OSX)
    ...
    should 'detect when a target should use the OS X profile' do
      #assert_equal(Profilers::OSX, @profiler.class)
    end
